...
$(document).ready(function () {
	...
	*/
	...
	$('#thumb3, #thumb3x').bind("click tap", function () {
		...
		$('#bigHallGallery').animate({top:670}, 500);
	});
	//
	$('#bigHallTop, #bigHallGalleryEnter').bind("click tap", function () {
		...
		$("#bigHallGallery").css("display", "block").animate({top:670}, 500);
	...
	$('#cl-101, #cl-102, #cl-103, #cl-factories, #cl-cab, #cl-callback, #cl-fprogram, #cl-fnconsproto, #cl-objcreate, #cl-jsonjq, #cl-jsonhttp').bind("click tap", function (event) {
		var id = $('#categories');
		var offset = 570; // px offset up or down
		var target = $(id).offset().top + offset;
		$('html, body').animate({
			scrollTop: target
		}, 500); //millisec, the higher the slower
	...
	$("#cl-101").bind("click tap", function () {
        fnAjax("/collections/javascript.php", "#ajax-home");
        $('#ajax-display').fadeIn(500);
	});
    
	$("#cl-102").bind("click tap", function () {
        fnAjax("/mysql/console-guestbook.php", "#ajax-home");
        $('#ajax-display').fadeIn(500);
	});
	...
	// To ajax in commentConsole file into #commentConsole from the top and bottom navbars.
	$("#thumb7, #thumb7x").bind("click tap", function(){
    $("#commentConsole").toggle(500); 
	//fnAjax("/mysql/view-comments.php", "#commentConsole");    
	});
	
	/*
	$('#commentConsole div span#close').bind("click tap", function(){
		$('#commentConsole').fadeOut(500);
	});
	*/
	
	/* Begin Development Only
	$("#applications").bind('change', function(){
		var selectedOption = $("#applications option:selected");
		if(selectedOption.val() === '2') {alert('2') // Comments
			fnAjax("/mysql/view-comments.php", "#commentConsole");
		};
		if(selectedOption.val() === '3') {alert('3') // Guestbook
			fnAjax("/mysql/view-guestbook.php", "#commentConsole");
		};
		if(selectedOption.val() === '4') {alert('4') // Bookstore
		fnAjax("/mysql/view-samples.php", "#commentConsole");
